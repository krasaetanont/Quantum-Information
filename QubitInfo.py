...
def mutual_info(P0, P1, rho0, rho1, ang, nbin):  # Function to find mutual information I
    x = np.array([])
    y = np.array([])
    z = np.array([0])
    ...
    for i in range(nbin + 1):
        ...
        x = np.append(x, a / mt.pi - ang / mt.pi)
        y = np.append(y, I)
...
def random_point(n, nbin):
    ...
    while (count < n):
        ...
        k = mutual_info(P0, P1, rho0, rho1, ang, nbin)
        if k[0] != 1:
            print(k[0], "rho0 = ", rho0, "rho1 = ", rho1, "P0 = ", P0)
            # plot(rho0, rho1, ang, P0, k)
            break
        # for finding multiple maximum point of the derivative of I
        # if k[3] != 1:
...
def runInParallel(nproc, num, nbin):
  proc = []
  for n in range(nproc):
    # p = Process(target=random_point(num))
    p = Process(target=random_point, args=(num, nbin))
...
if __name__ == "__main__":
    runInParallel(11, 1000, 1000) # first arg = number of processor, second arg = number of iteration, third arg = number of angle precision
